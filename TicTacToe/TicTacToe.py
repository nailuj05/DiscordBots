...
@bot.event
async def on_ready():
    await bot.change_presence(status=discord.Status.online, activity=discord.Game('TicTacToe'))
    print('Bot online')
...
async def play(ctx, row: int, collumn: int):
    ...
    if (gameWon != True):
        ...
        embed.add_field(name="Diskrepanz!",
                        value=(Plot(board) + "Das Spiel ist bereits gespielt, erstelle ein neues Spiel mit /newGame"),
                        inline=True)
        await ctx.send(embed=embed)
